...
async function processArea(area: 'a' | 'b' | 'e' | 'z') {
  console.log(`\n${'='.repeat(60)}`);
  console.log(`🏛️  Processing Oblivion City Paru - Area ${area.toUpperCase()}`);
  ...
  const outputDir = join(OUTPUT_DIR, `paru_${area}`);
  ...
  console.log(`\n${'='.repeat(60)}`);
  console.log(`📊 Summary for Paru ${area.toUpperCase()}`);
  console.log('='.repeat(60));
...
async function main() {
  console.log('🚀 PSZ Oblivion City Paru Processor (Scene 05)');
  console.log('Converting Paru NARC/NSBTX files to GLB format\n');
  ...
  console.log(`   - ${join(OUTPUT_DIR, 'paru_a')}`);
  console.log(`   - ${join(OUTPUT_DIR, 'paru_b')}`);
  console.log(`   - ${join(OUTPUT_DIR, 'paru_e')}`);
  console.log(`   - ${join(OUTPUT_DIR, 'paru_z')}`);
}
