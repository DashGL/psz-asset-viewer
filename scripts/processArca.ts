...
async function processArea(area: 'a' | 'b' | 'e' | 'z') {
  console.log(`\n${'='.repeat(60)}`);
  console.log(`🏭 Processing Arca Plant - Area ${area.toUpperCase()}`);
  ...
  const outputDir = join(OUTPUT_DIR, `arca_${area}`);
  ...
  console.log(`\n${'='.repeat(60)}`);
  console.log(`📊 Summary for Arca Plant ${area.toUpperCase()}`);
  console.log('='.repeat(60));
...
async function main() {
  console.log('🚀 PSZ Arca Plant Processor (Scene 06)');
  console.log('Converting Arca Plant NARC/NSBTX files to GLB format\n');
  ...
  console.log(`   - ${join(OUTPUT_DIR, 'arca_a')}`);
  console.log(`   - ${join(OUTPUT_DIR, 'arca_b')}`);
  console.log(`   - ${join(OUTPUT_DIR, 'arca_e')}`);
  console.log(`   - ${join(OUTPUT_DIR, 'arca_z')}`);
}
