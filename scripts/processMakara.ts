...
async function processArea(area: 'a' | 'b' | 'e' | 'z') {
  console.log(`\n${'='.repeat(60)}`);
  console.log(`🏛️  Processing Makara Ruins - Area ${area.toUpperCase()}`);
  ...
  const outputDir = join(OUTPUT_DIR, `makara_${area}`);
  ...
  console.log(`\n${'='.repeat(60)}`);
  console.log(`📊 Summary for Makara ${area.toUpperCase()}`);
  console.log('='.repeat(60));
...
async function main() {
  console.log('🚀 PSZ Makara Ruins Processor (Scene 04)');
  console.log('Converting Makara Ruins NARC/NSBTX files to GLB format\n');
  ...
  console.log(`   - ${join(OUTPUT_DIR, 'makara_a')}`);
  console.log(`   - ${join(OUTPUT_DIR, 'makara_b')}`);
  console.log(`   - ${join(OUTPUT_DIR, 'makara_e')}`);
  console.log(`   - ${join(OUTPUT_DIR, 'makara_z')}`);
}
