...
async function main() {
  console.log('🚀 PSZ Enemy Processor (TEST)');
  console.log('Converting Sample Enemy NARC files to GLB format\n');

  // Only process the three test enemies
  const testEnemies = ['frog', 'rappy', 'jigobooma'];
  const enemyFiles = testEnemies.map(name => join(ENEMY_DIR, `${name}.narc`));

  console.log(`Processing ${enemyFiles.length} test enemies\n`);
