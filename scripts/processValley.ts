...
async function processArea(area: 'a' | 'b' | 'e' | 'z') {
  console.log(`\n${'='.repeat(60)}`);
  console.log(`🏔️  Processing Gurhacia Valley - Area ${area.toUpperCase()}`);
  ...
  const outputDir = join(OUTPUT_DIR, `valley_${area}`);
  ...
  console.log(`\n${'='.repeat(60)}`);
  console.log(`📊 Summary for Valley ${area.toUpperCase()}`);
  console.log('='.repeat(60));
...
async function main() {
  console.log('🚀 PSZ Gurhacia Valley Processor (Scene 01)');
  console.log('Converting Valley NARC/NSBTX files to GLB format\n');
  ...
  console.log(`   - ${join(OUTPUT_DIR, 'valley_a')}`);
  console.log(`   - ${join(OUTPUT_DIR, 'valley_b')}`);
  console.log(`   - ${join(OUTPUT_DIR, 'valley_e')}`);
  console.log(`   - ${join(OUTPUT_DIR, 'valley_z')}`);
}
