...
async function processArea(area: 'a' | 'b' | 'e' | 'z') {
  console.log(`\n${'='.repeat(60)}`);
  console.log(`⛩️  Processing Dark Shrine - Area ${area.toUpperCase()}`);
  ...
  const outputDir = join(OUTPUT_DIR, `shrine_${area}`);
  ...
  console.log(`\n${'='.repeat(60)}`);
  console.log(`📊 Summary for Dark Shrine ${area.toUpperCase()}`);
  console.log('='.repeat(60));
...
async function main() {
  console.log('🚀 PSZ Dark Shrine Processor (Scene 07)');
  console.log('Converting Dark Shrine NARC/NSBTX files to GLB format\n');
  ...
  console.log(`   - ${join(OUTPUT_DIR, 'shrine_a')}`);
  console.log(`   - ${join(OUTPUT_DIR, 'shrine_b')}`);
  console.log(`   - ${join(OUTPUT_DIR, 'shrine_e')}`);
  console.log(`   - ${join(OUTPUT_DIR, 'shrine_z')}`);
}
